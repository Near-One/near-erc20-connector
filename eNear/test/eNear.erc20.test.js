...
contract('eNear ERC20 behaviour', function (accounts) {
  ...
  it('has 24 decimals', async function () {
    expect(await this.token.decimals()).to.be.bignumber.equal('24');
  });
  ...
  describe('_mint', function () {
    ...
    it('rejects a null account', async function () {
      await expectRevert(
        this.token.mintTo(ZERO_ADDRESS, amount, {from: minter}), 'ERC20: mint to the zero address',
      );
    });

    describe('for a non zero account', function () {
      beforeEach('minting', async function () {
        const { logs } = await this.token.mintTo(recipient, amount, {from: minter});
        this.logs = logs;
      });
